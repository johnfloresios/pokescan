...
function CameraScreen({ cameraRef, onClose, onCapture, error }: any) {
  return <View style={s.cameraPage}><CameraView ref={cameraRef} style={StyleSheet.absoluteFill} facing="back" autofocus="on" />
    ...
    <SafeAreaView style={s.cameraSafe}><View style={s.cameraTop}><Pressable onPress={onClose} style={s.glassButton}><Feather name="x" size={23} color={C.white} /></Pressable><Text style={s.cameraTitle}>Scan your card</Text><View style={s.glassButton}><Feather name="zap" size={20} color={C.white} /></View></View>
      ...
      <View><Text style={s.cameraHelp}>Hold steady and avoid glare</Text>{!!error && <Text style={s.cameraError}>{error}</Text>}<View style={s.shutterRow}><View style={{width:48}} /><Pressable onPress={onCapture} style={s.shutterOuter}><View style={s.shutterInner} /></Pressable><Pressable style={s.miniRound}><Feather name="image" size={21} color={C.white} /></Pressable></View></View>
    </SafeAreaView>
  </View>;
}
...
function Detail({card,onBack}:{card:Card;onBack:()=>void}) { const p=card.prices[0]; return <View style={s.page}><ScrollView showsVerticalScrollIndicator={false}><LinearGradient colors={['#4C1D95',C.ink]} style={s.detailHero}><SafeAreaView><View style={s.detailTop}><Pressable onPress={onBack} style={s.glassButton}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><Text style={s.detailNav}>Card details</Text><Pressable style={s.glassButton}><Feather name="share-2" size={20} color={C.white}/></Pressable></View><View style={s.cardGlow}/><Image source={cardImageSource(card)} style={s.heroCard} contentFit="contain" transition={300}/></SafeAreaView></LinearGradient>
    <View style={s.detailBody}><View style={s.detailHeading}><View style={{flex:1}}><Text style={s.detailName}>{card.name}</Text><Text style={s.detailSet}>{card.setName} · {card.number}</Text></View><View style={s.typeBadge}><Feather name="zap" size={14} color={C.ink}/><Text style={s.typeText}>{card.type}</Text></View></View>
      ...
      <Text style={s.sectionTitle}>CARD INFORMATION</Text><View style={s.infoGrid}><Info label="RARITY" value={card.rarity}/><Info label="CARD NO." value={card.number}/><Info label="STAGE" value={card.stage||'—'}/><Info label="HP" value={card.hp||'—'}/></View>
      {(card.attacks?.length||card.text)&&<><Text style={s.sectionTitle}>CARD DETAILS</Text><View style={s.detailsBox}>{card.attacks?.map(a=><View key={a} style={s.attack}><View style={s.energyDot}/><Text style={s.attackText}>{a}</Text></View>)}{!!card.text&&<Text style={s.cardText}>{card.text}</Text>}{!!card.weakness&&<Text style={s.weakness}>Weakness  ·  {card.weakness}</Text>}</View></>}
