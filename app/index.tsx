...
import { ActivityIndicator, Alert, Animated, Dimensions, FlatList, ImageBackground, KeyboardAvoidingView, PanResponder, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View } from 'react-native';
...
const VARIANTS=['Normal','Holo','Reverse Holo','Illustration Rare','Special Illustration Rare','Hyper Rare','1st Edition'];
const RARITIES=['Common','Uncommon','Rare','Double Rare','Illustration Rare','Special Illustration Rare','Hyper Rare'];
...
function Sparkline({positive=true}:{positive?:boolean}) {
  const values=positive?[32,29,35,33,41,39,48,45,54,57,53,63]:[63,60,55,58,49,51,43,46,38,35,32,29];
  const width=Math.min(W-82,330),step=width/(values.length-1);
  return <View style={[s.sparkline,{width}]}>{values.slice(0,-1).map((value,index)=>{const next=values[index+1],dx=step,dy=next-value,length=Math.sqrt(dx*dx+dy*dy),angle=Math.atan2(dy,dx)*180/Math.PI;return <View key={index} style={[s.sparkSegment,{backgroundColor:positive?C.green:C.red,width:length,left:index*step,top:68-value,transform:[{rotate:`${angle}deg`}]}]}/>;})}</View>;
}
...
function SwipeToDelete({children,onDelete}:{children:any;onDelete:()=>void}) {
  ...
  const settle=(open:boolean)=>Animated.spring(x,{toValue:open?-86:0,useNativeDriver:true,bounciness:0,speed:22}).start();
  const responder=useRef(PanResponder.create({
    onMoveShouldSetPanResponder:(_event,gesture)=>Math.abs(gesture.dx)>10&&Math.abs(gesture.dx)>Math.abs(gesture.dy),
    onPanResponderMove:(_event,gesture)=>x.setValue(Math.max(-96,Math.min(0,gesture.dx))),
    onPanResponderRelease:(_event,gesture)=>settle(gesture.dx<-48),
...
function CollectionScreen({userId,onBack,onScan,onSelect}:{userId:string;onBack:()=>void;onScan:()=>void;onSelect:(row:ScannedCardRow)=>void}) {
  ...
  const header=<View><View style={s.portfolioHero}><View style={s.portfolioTopLine}><View><Text style={s.portfolioLabel}>TOTAL PORTFOLIO VALUE</Text><Text style={s.portfolioAmount}>{money(portfolioValue)}</Text><Text style={[s.portfolioDelta,{color:portfolioChange>=0?C.green:C.red}]}>{portfolioChange>=0?'+':''}{money(portfolioChangeDollars)} ({portfolioChange>=0?'+':''}{portfolioChange.toFixed(1)}%) today</Text></View><View style={s.dayChange}><Feather name="arrow-up-right" size={14} color={C.green}/><Text style={s.dayChangePeriod}>24H</Text></View></View><Sparkline positive={portfolioChange>=0}/><View style={s.portfolioStats}><Text style={s.portfolioStat}><Text style={s.portfolioStatStrong}>{copyCount}</Text> cards</Text><View style={s.statDot}/><Text style={s.portfolioStat}><Text style={s.portfolioStatStrong}>{uniqueCount}</Text> unique</Text><View style={s.statDot}/><Text style={s.portfolioStat}><Text style={s.portfolioStatStrong}>{setCount}</Text> sets</Text></View></View>
    ...
    <Text style={s.filterLabel}>RARITY</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}><CollectionChip label="All rarities" active={rarity==='All'} onPress={()=>setRarity('All')}/>{RARITIES.map(value=><CollectionChip key={value} label={value} active={rarity===value} onPress={()=>setRarity(value)}/>)}</ScrollView>
...
function SavedCardControls({row,onUpdate,onDelete}:{row:ScannedCardRow;onUpdate:(updates:Pick<ScannedCardRow,'quantity'|'condition'|'variant'|'notes'>)=>Promise<void>;onDelete:()=>Promise<void>}) {
  const [quantity,setQuantity]=useState(Math.max(1,Number(row.quantity)||1));const [condition,setCondition]=useState(row.condition||'NM');const [variant,setVariant]=useState(row.variant||'Normal');const [notes,setNotes]=useState(row.notes||'');const [saving,setSaving]=useState(false);const [message,setMessage]=useState('');
  useEffect(()=>{setQuantity(Math.max(1,Number(row.quantity)||1));setCondition(row.condition||'NM');setVariant(row.variant||'Normal');setNotes(row.notes||'');},[row]);
  const save=async()=>{setSaving(true);setMessage('');try{await onUpdate({quantity,condition,variant,notes:notes.trim()||null});setMessage('Collection details saved.');void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);}catch(e){setMessage(e instanceof Error?e.message:'Could not update card.');}finally{setSaving(false);}};
  const remove=()=>Alert.alert('Delete saved card?',`${row.card_name} will be removed from your collection.`,[{text:'Cancel',style:'cancel'},{text:'Delete',style:'destructive',onPress:async()=>{setSaving(true);try{await onDelete();}catch(e){setMessage(e instanceof Error?e.message:'Could not delete card.');setSaving(false);}}}]);
  return <View style={s.savedControls}><View style={s.savedControlsTitle}><View><Text style={s.smallHead}>IN YOUR COLLECTION</Text><Text style={s.savedHeading}>Collection details</Text></View><Feather name="check-circle" size={22} color={C.green}/></View><Text style={s.editLabel}>QUANTITY</Text><View style={s.quantityControl}><Pressable onPress={()=>setQuantity(value=>Math.max(1,value-1))} style={s.quantityButton}><Feather name="minus" size={20} color={C.white}/></Pressable><Text style={s.quantityValue}>{quantity}</Text><Pressable onPress={()=>setQuantity(value=>Math.min(999,value+1))} style={s.quantityButton}><Feather name="plus" size={20} color={C.white}/></Pressable></View><Text style={s.editLabel}>CONDITION</Text><View style={s.editChipWrap}>{CONDITIONS.map(value=><CollectionChip key={value} label={value} active={condition===value} onPress={()=>setCondition(value)}/>)}</View><Text style={s.editLabel}>VARIANT</Text><View style={s.editChipWrap}>{VARIANTS.map(value=><CollectionChip key={value} label={value} active={variant===value} onPress={()=>setVariant(value)}/>)}</View><Text style={s.editLabel}>NOTES</Text><TextInput value={notes} onChangeText={setNotes} multiline maxLength={500} placeholder="Add collection notes…" placeholderTextColor="#71839C" selectionColor={C.cyan} style={s.notesInput}/>{!!message&&<Text style={s.savedMessage}>{message}</Text>}<View style={s.editActions}><Pressable onPress={remove} disabled={saving} style={s.deleteButton}><Feather name="trash-2" size={17} color="#FF98A2"/><Text style={s.deleteButtonText}>Delete</Text></Pressable><Pressable onPress={save} disabled={saving} style={s.saveEditButton}>{saving?<ActivityIndicator color={C.ink}/>:<Text style={s.saveEditText}>SAVE DETAILS</Text>}</Pressable></View></View>;
}

function Detail({card,savedRow,onBack,onHome,onCollection,onSave,onUpdateSaved,onDeleteSaved,onScan,initiallySaved=false}:{card:Card;savedRow:ScannedCardRow|null;onBack:()=>void;onHome:()=>void;onCollection:()=>void;onSave:(card:Card)=>Promise<ScannedCardRow>;onUpdateSaved:(updates:Pick<ScannedCardRow,'quantity'|'condition'|'variant'|'notes'>)=>Promise<void>;onDeleteSaved:()=>Promise<void>;onScan:()=>void;initiallySaved?:boolean}) { const p=card.prices[0]; const [saving,setSaving]=useState(false); const [saved,setSaved]=useState(initiallySaved); const [managedRow,setManagedRow]=useState(savedRow); const [saveError,setSaveError]=useState(''); useEffect(()=>setManagedRow(savedRow),[savedRow]); const save=async()=>{if(saving||saved)return;setSaving(true);setSaveError('');try{const row=await onSave(card);setManagedRow(row);setSaved(true);void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);}catch(e){setSaveError(e instanceof Error?e.message:'Could not save card.');}finally{setSaving(false);}}; return <View style={s.page}><ScrollView showsVerticalScrollIndicator={false}><LinearGradient colors={['#4C1D95',C.ink]} style={s.detailHero}><SafeAreaView><View style={s.detailTop}><Pressable onPress={onBack} style={s.glassButton}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><Text style={s.detailNav}>Card details</Text><TierBadge/></View><View style={s.cardGlow}/><Image source={cardImageSource(card)} style={s.heroCard} contentFit="contain" transition={300}/></SafeAreaView></LinearGradient>
    <View style={s.detailBody}><Text style={s.matchedIdentifier}>MATCHED: {card.name} ({card.setCode} {card.number})</Text><View style={s.detailHeading}><View style={{flex:1}}><Text style={s.detailName}>{card.name}</Text><Text style={s.detailSet}>{card.setName} · {card.setCode} {card.number}</Text></View><View style={s.typeBadge}><Feather name="zap" size={14} color={C.ink}/><Text style={s.typeText}>{card.type}</Text></View></View>
      ...
      {!!saveError&&<Text style={s.saveError}>{saveError}</Text>}{managedRow?<SavedCardControls row={managedRow} onUpdate={async updates=>{await onUpdateSaved(updates);setManagedRow(current=>current?{...current,...updates}:current);}} onDelete={onDeleteSaved}/>:<Pressable onPress={save} disabled={saving} style={({pressed})=>[s.collection,(pressed||saving)&&{opacity:.72}]}>{saving?<ActivityIndicator color={C.ink}/>:<><Feather name="plus" size={20} color={C.ink}/><Text style={s.collectionText}>Add to collection</Text></>}</Pressable>}<Text style={s.disclaimer}>Prices are estimates and vary by condition, language, and market.</Text>
