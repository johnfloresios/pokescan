...
import { ActivityIndicator, Alert, Animated, Dimensions, FlatList, ImageBackground, KeyboardAvoidingView, Modal, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View } from 'react-native';
...
function CollectionScreen({userId,onBack,onScan,onSelect}:{userId:string;onBack:()=>void;onScan:()=>void;onSelect:(row:ScannedCardRow)=>void}) {
  ...
  const startEdit=(card:ScannedCardRow)=>{setEditing(card);setEditQuantity(Math.max(1,Number(card.quantity)||1));setEditCondition(({ 'Near Mint':'NM','Lightly Played':'LP','Moderately Played':'MP','Heavily Played':'HP','Damaged':'DMG' } as Record<string,string>)[card.condition||'']||card.condition||'NM');setEditVariant(card.variant||'Normal');setEditNotes(card.notes||'');};
  const saveEdit=async()=>{if(!editing)return;setSaving(true);const {error}=await supabase.from('scanned_cards').update({quantity:editQuantity,condition:editCondition,variant:editVariant,notes:editNotes.trim()||null}).eq('id',editing.id).eq('user_id',userId);setSaving(false);if(error){Alert.alert('Could not update card',error.message);return;}setEditing(null);await load(true);};
