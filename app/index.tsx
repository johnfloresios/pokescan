...
function CollectionScreen({userId,onBack,onScan,onSelect}:{userId:string;onBack:()=>void;onScan:()=>void;onSelect:(row:ScannedCardRow)=>void}) {
  ...
  return <View style={s.page}><SafeAreaView style={s.safe}><View style={s.collectionTop}><Pressable onPress={onBack} style={s.back}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><View style={{flex:1}}><Text style={s.smallHead}>YOUR SAVED CARDS</Text><Text style={s.resultTitle}>Collection</Text></View><Pressable onPress={onScan} style={s.topScan}><MaterialCommunityIcons name="line-scan" size={21} color={C.ink}/></Pressable></View>
...
function Matches({ query, cards, onBack, onSelect, onSearch, onScan }: {query:string;cards:Card[];onBack:()=>void;onSelect:(c:Card)=>void;onSearch:(q:string)=>void;onScan:()=>void}) {
  return <View style={s.page}><SafeAreaView style={s.safe}><View style={s.resultTop}><Pressable onPress={onBack} style={s.back}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><View style={{flex:1}}><Text style={s.smallHead}>SCAN COMPLETE</Text><Text style={s.resultTitle}>Choose your match</Text></View><Pressable onPress={onScan} style={s.topScan}><MaterialCommunityIcons name="line-scan" size={21} color={C.ink}/></Pressable></View>
...
function Detail({card,onBack,onSave,onScan,initiallySaved=false}:{card:Card;onBack:()=>void;onSave:(card:Card)=>Promise<void>;onScan:()=>void;initiallySaved?:boolean}) { const p=card.prices[0]; const [saving,setSaving]=useState(false); const [saved,setSaved]=useState(initiallySaved); const [saveError,setSaveError]=useState(''); const save=async()=>{if(saving||saved)return;setSaving(true);setSaveError('');try{await onSave(card);setSaved(true);void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);}catch(e){setSaveError(e instanceof Error?e.message:'Could not save card.');}finally{setSaving(false);}}; return <View style={s.page}><ScrollView showsVerticalScrollIndicator={false}><LinearGradient colors={['#4C1D95',C.ink]} style={s.detailHero}><SafeAreaView><View style={s.detailTop}><Pressable onPress={onBack} style={s.glassButton}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><Text style={s.detailNav}>Card details</Text><Pressable onPress={onScan} style={s.glassButton}><MaterialCommunityIcons name="line-scan" size={21} color={C.white}/></Pressable></View><View style={s.cardGlow}/><Image source={cardImageSource(card)} style={s.heroCard} contentFit="contain" transition={300}/></SafeAreaView></LinearGradient>
