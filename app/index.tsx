...
import { ActivityIndicator, Alert, Animated, Dimensions, ImageBackground, KeyboardAvoidingView, Modal, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View } from 'react-native';
...
type ScannedCardRow = { id:string;card_name:string;set_name:string;set_number:string;image_url:string|null;price_estimate:number|string|null;quantity:number|null;condition:string|null;variant:string|null;created_at:string };
...
function Home({ userId, name, email, onScan, onCollection, onSelectCard, onSearch, onLogout, error }: { userId:string;name:string;email:string;onScan:()=>void;onCollection:()=>void;onSelectCard:(row:ScannedCardRow)=>void;onSearch:(q:string)=>void;onLogout:()=>void;error:string }) {
  ...
  const loadDashboard=useCallback(async()=>{
    ...
    const [values,recent]=await Promise.all([
      ...
      supabase.from('scanned_cards').select('id,card_name,set_name,set_number,image_url,price_estimate,quantity,condition,variant,created_at').eq('user_id',userId).order('created_at',{ascending:false}).limit(5),
    ]);
...
const VARIANTS=['Normal','Holo','Reverse Holo','1st Edition'];
type CollectionSort='newest'|'value'|'name';
...
function CollectionScreen({userId,onBack,onScan,onSelect}:{userId:string;onBack:()=>void;onScan:()=>void;onSelect:(row:ScannedCardRow)=>void}) {
  ...
  const [editing,setEditing]=useState<ScannedCardRow|null>(null);
  ...
  const load=useCallback(async(reset=true)=>{
    ...
    let request=supabase.from('scanned_cards').select('id,card_name,set_name,set_number,image_url,price_estimate,quantity,condition,variant,created_at',{count:'exact'}).eq('user_id',userId);
    ...
    if(sort==='value')request=request.order('price_estimate',{ascending:false});
    else if(sort==='name')request=request.order('card_name',{ascending:true});
    else request=request.order('created_at',{ascending:false});
    const {data,error,count}=await request.range(offset,offset+COLLECTION_PAGE_SIZE-1);
    if(error)setLoadError(error.message);
    else {const next=(data??[]) as ScannedCardRow[];setCards(current=>reset?next:[...current,...next]);setRowCount(count??next.length);}
    if(reset){
      const summary=await supabase.from('scanned_cards').select('price_estimate,quantity').eq('user_id',userId);
      if(!summary.error){const rows=(summary.data??[]) as Array<{price_estimate:number|string|null;quantity:number|null}>;setCopyCount(rows.reduce((sum,row)=>sum+Math.max(1,Number(row.quantity)||1),0));setPortfolioValue(rows.reduce((sum,row)=>sum+(Number(row.price_estimate)||0)*Math.max(1,Number(row.quantity)||1),0));}
    }
    setLoading(false);setLoadingMore(false);
  },[cards.length,condition,search,sort,userId,variant]);

  useEffect(()=>{const timer=setTimeout(()=>{void load(true);},search?300:0);return()=>clearTimeout(timer);},[condition,search,sort,userId,variant]);
  const startEdit=(card:ScannedCardRow)=>{setEditing(card);setEditQuantity(Math.max(1,Number(card.quantity)||1));setEditCondition(card.condition||'Near Mint');setEditVariant(card.variant||'Normal');};
  const saveEdit=async()=>{if(!editing)return;setSaving(true);const {error}=await supabase.from('scanned_cards').update({quantity:editQuantity,condition:editCondition,variant:editVariant}).eq('id',editing.id).eq('user_id',userId);setSaving(false);if(error){Alert.alert('Could not update card',error.message);return;}setEditing(null);await load(true);};
  const deleteCard=()=>{if(!editing)return;Alert.alert('Delete saved card?',`${editing.card_name} will be removed from your collection.`,[{text:'Cancel',style:'cancel'},{text:'Delete',style:'destructive',onPress:async()=>{setSaving(true);const {error}=await supabase.from('scanned_cards').delete().eq('id',editing.id).eq('user_id',userId);setSaving(false);if(error){Alert.alert('Could not delete card',error.message);return;}setEditing(null);await load(true);}}]);};
  ...
  return <View style={s.page}><SafeAreaView style={s.safe}><View style={s.collectionTop}><Pressable onPress={onBack} style={s.back}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><View style={{flex:1}}><Text style={s.smallHead}>YOUR SAVED CARDS</Text><Text style={s.resultTitle}>Collection</Text></View></View>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.collectionScroll} refreshControl={<RefreshControl refreshing={loading} onRefresh={()=>load(true)} tintColor={C.cyan}/>}><View style={s.collectionSummary}><View><Text style={s.metricLabel}>PORTFOLIO VALUE</Text><Text style={s.collectionTotal}>{money(portfolioValue)}</Text></View><View style={s.collectionCount}><Text style={s.collectionCountValue}>{copyCount}</Text><Text style={s.metricHint}>total copies</Text></View></View>
      ...
      <Text style={s.filterLabel}>SORT</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}><CollectionChip label="Newest" active={sort==='newest'} onPress={()=>setSort('newest')}/><CollectionChip label="Highest value" active={sort==='value'} onPress={()=>setSort('value')}/><CollectionChip label="A–Z" active={sort==='name'} onPress={()=>setSort('name')}/></ScrollView>
      <Text style={s.filterLabel}>CONDITION</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}><CollectionChip label="All" active={condition==='All'} onPress={()=>setCondition('All')}/>{CONDITIONS.map(value=><CollectionChip key={value} label={value} active={condition===value} onPress={()=>setCondition(value)}/>)}</ScrollView>
      <Text style={s.filterLabel}>VARIANT</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}><CollectionChip label="All" active={variant==='All'} onPress={()=>setVariant('All')}/>{VARIANTS.map(value=><CollectionChip key={value} label={value} active={variant===value} onPress={()=>setVariant(value)}/>)}</ScrollView>
      <View style={s.collectionHeading}><Text style={s.dashboardSection}>CARDS</Text><Text style={s.collectionLimit}>{rowCount} saved entries</Text></View>
      {!!loadError&&<Text style={s.error}>{loadError}</Text>}{!loading&&!cards.length?<View style={s.recentEmpty}><MaterialCommunityIcons name="cards-outline" size={40} color={C.muted}/><Text style={s.emptyTitle}>{search||condition!=='All'||variant!=='All'?'No matching cards':'Your collection is empty'}</Text><Text style={s.emptySub}>{search||condition!=='All'||variant!=='All'?'Try clearing a search or filter.':'Scan a card, choose a match, and save it here.'}</Text>{!search&&condition==='All'&&variant==='All'&&<Pressable onPress={onScan} style={s.emptyScan}><Text style={s.emptyScanText}>SCAN A CARD</Text></Pressable>}</View>:cards.map(card=><Pressable key={card.id} onPress={()=>onSelect(card)} style={({pressed})=>[s.collectionRow,pressed&&{opacity:.75}]}>{card.image_url?<Image source={{uri:card.image_url}} style={s.collectionImage} contentFit="cover"/>:<View style={[s.collectionImage,s.recentPlaceholder]}><MaterialCommunityIcons name="cards" size={24} color={C.muted}/></View>}<View style={s.recentInfo}><View style={s.collectionNameLine}><Text style={s.collectionName} numberOfLines={1}>{card.card_name}</Text><Text style={s.quantityBadge}>×{Math.max(1,Number(card.quantity)||1)}</Text></View><Text style={s.recentSet} numberOfLines={1}>{card.set_name}</Text><Text style={s.collectionNumber}>{card.set_number}</Text><Text style={s.collectionMeta}>{card.condition||'Near Mint'} · {card.variant||'Normal'}</Text></View><View style={s.collectionPriceWrap}><Text style={s.recentPrice}>{money((Number(card.price_estimate)||0)*Math.max(1,Number(card.quantity)||1))}</Text><Pressable onPress={event=>{event.stopPropagation();startEdit(card);}} hitSlop={8} style={s.editCard}><Feather name="edit-2" size={15} color={C.cyan}/></Pressable></View></Pressable>)}
      {hasMore&&<Pressable onPress={()=>load(false)} disabled={loadingMore} style={s.loadMore}>{loadingMore?<ActivityIndicator color={C.ink}/>:<Text style={s.loadMoreText}>LOAD MORE</Text>}</Pressable>}</ScrollView>
    <BottomNav active="collection" onHome={onBack} onCollection={()=>{}} onScan={onScan}/></SafeAreaView>
    <Modal visible={!!editing} transparent animationType="slide" onRequestClose={()=>!saving&&setEditing(null)}><View style={s.modalShade}><KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':undefined} style={s.editSheet}><View style={s.sheetHandle}/><View style={s.editHeader}><View style={{flex:1}}><Text style={s.smallHead}>EDIT SAVED CARD</Text><Text style={s.editTitle}>{editing?.card_name}</Text></View><Pressable onPress={()=>setEditing(null)} disabled={saving} style={s.modalClose}><Feather name="x" size={20} color={C.white}/></Pressable></View><Text style={s.editLabel}>QUANTITY</Text><View style={s.quantityControl}><Pressable onPress={()=>setEditQuantity(value=>Math.max(1,value-1))} style={s.quantityButton}><Feather name="minus" size={20} color={C.white}/></Pressable><Text style={s.quantityValue}>{editQuantity}</Text><Pressable onPress={()=>setEditQuantity(value=>Math.min(999,value+1))} style={s.quantityButton}><Feather name="plus" size={20} color={C.white}/></Pressable></View><Text style={s.editLabel}>CONDITION</Text><View style={s.editChipWrap}>{CONDITIONS.map(value=><CollectionChip key={value} label={value} active={editCondition===value} onPress={()=>setEditCondition(value)}/>)}</View><Text style={s.editLabel}>VARIANT</Text><View style={s.editChipWrap}>{VARIANTS.map(value=><CollectionChip key={value} label={value} active={editVariant===value} onPress={()=>setEditVariant(value)}/>)}</View><View style={s.editActions}><Pressable onPress={deleteCard} disabled={saving} style={s.deleteButton}><Feather name="trash-2" size={17} color="#FF98A2"/><Text style={s.deleteButtonText}>Delete</Text></Pressable><Pressable onPress={saveEdit} disabled={saving} style={s.saveEditButton}>{saving?<ActivityIndicator color={C.ink}/>:<Text style={s.saveEditText}>SAVE CHANGES</Text>}</Pressable></View></KeyboardAvoidingView></View></Modal></View>;
}
