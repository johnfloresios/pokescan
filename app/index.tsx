...
export default function App() {
  ...
  if (screen === 'detail' && selected) return <Detail card={selected} onBack={() => setScreen('matches')} onSave={saveToCollection} />;
  return <Home userId={session.user.id} email={session.user.email ?? 'Collector'} onScan={openCamera} onSearch={lookup} onLogout={() => supabase.auth.signOut()} error={error} />;
}
...
function Home({ userId, email, onScan, onSearch, onLogout, error }: { userId:string;email:string;onScan:()=>void;onSearch:(q:string)=>void;onLogout:()=>void;error:string }) {
  ...
  return <View style={s.page}>
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.dashboardScroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" refreshControl={<RefreshControl refreshing={loading} onRefresh={loadDashboard} tintColor={C.cyan}/>}>
        ...
        <View style={s.sectionRow}><Text style={s.dashboardSection}>RECENT SCANS</Text><Text style={s.seeAll}>View collection</Text></View>
      ...
      <View style={s.bottomNav}><View style={s.navItem}><Feather name="home" size={20} color={C.yellow}/><Text style={[s.navText,{color:C.yellow}]}>Home</Text></View><View style={s.navItem}><Feather name="layers" size={20} color={C.muted}/><Text style={s.navText}>Collection</Text></View><Pressable onPress={onScan} style={s.navScan}><MaterialCommunityIcons name="line-scan" size={25} color={C.ink}/></Pressable></View>
    </SafeAreaView>
...
const s=StyleSheet.create({
  ...
  saveError:{fontFamily:'Inter_500Medium',fontSize:11,color:'#FF9EA8',textAlign:'center',marginTop:20},collectionSaved:{backgroundColor:C.green},
