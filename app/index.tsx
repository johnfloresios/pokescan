...
type Screen = 'home' | 'collection' | 'camera' | 'analyzing' | 'confirm' | 'matches' | 'detail';
type ScanDraft = { imageUri:string;name:string;set:string;number:string;scan:ScanText };
...
export default function App() {
  ...
  const lookup = async (q: string, uri?: string) => {
    try {
      ...
      if (uri) {
        ...
        setScanDraft({imageUri:uri,name:scan.hints.name??'',set:scan.hints.setCode??scan.hints.setName??'',number:scan.hints.number??scan.hints.bottomIdentifier??'',scan});
        setConfirmationError('');setScreen('confirm');return;
      }
      await searchWithScan(q.trim());
    } catch (e) { setError(e instanceof Error ? e.message : 'Something went wrong.'); setScreen(uri ? 'camera' : 'home'); }
  };
  const confirmScan=async(draft:ScanDraft)=>{
    const name=draft.name.trim(),set=draft.set.trim(),number=draft.number.trim();
    const queries=[set&&number?`${set} ${number}`:'',name&&set?`${name} ${set}`:'',name&&number?`${name} ${number}`:'',name].filter((value,index,all):value is string=>!!value&&all.indexOf(value)===index);
    if(!queries.length){setConfirmationError('Enter a card name, set, or collector number.');return;}
    const setIsCode=/^[a-z0-9-]{2,8}$/i.test(set)&&!set.includes(' ');
    const corrected:ScanText={...draft.scan,query:queries[0],queries,hints:{...draft.scan.hints,name:name||undefined,number:number||undefined,bottomIdentifier:number||undefined,setCode:setIsCode?set:undefined,setName:set&&!setIsCode?set:undefined}};
    setScanDraft({...draft,name,set,number});setConfirmationError('');
    try{await searchWithScan(queries[0],corrected);}catch(e){setConfirmationError(e instanceof Error?e.message:'Could not find this card.');setScreen('confirm');}
  };
  ...
  if (screen === 'confirm'&&scanDraft) return <ScanConfirmation draft={scanDraft} error={confirmationError} onChange={setScanDraft} onConfirm={confirmScan} onRescan={()=>{setScanDraft(null);void openCamera();}} onCancel={()=>{setScanDraft(null);setScreen('home');}}/>;
...
function HeaderIdentity({title}:{title?:string}) { return <View style={s.headerIdentity}><Brand/>{!!title&&<Text style={s.headerPageTitle}>{title}</Text>}</View>; }
...
function Home({ userId, name, email, onScan, onCollection, onSelectCard, onSearch, onLogout, error }: { userId:string;name:string;email:string;onScan:()=>void;onCollection:()=>void;onSelectCard:(row:ScannedCardRow)=>void;onSearch:(q:string)=>void;onLogout:()=>void;error:string }) {
  ...
  return <View style={s.page}>
    ...
    <SafeAreaView style={s.safe}>
      <View style={s.top}><Brand /><View style={s.topActions}><TierBadge/><Pressable onPress={onLogout} style={s.logout}><Feather name="log-out" size={15} color={C.white}/><Text style={s.logoutText}>Log out</Text></Pressable></View></View>
...
function CollectionScreen({userId,onBack,onScan,onSelect}:{userId:string;onBack:()=>void;onScan:()=>void;onSelect:(row:ScannedCardRow)=>void}) {
  ...
  return <View style={s.page}><SafeAreaView style={s.safe}><View style={s.collectionTop}><Pressable onPress={onBack} style={s.back}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><HeaderIdentity title="Collection"/><TierBadge/></View>
...
function CameraScreen({ onClose, onPhoto, error }: any) {
  ...
  return <View style={s.cameraPage}><Camera ref={camera} style={StyleSheet.absoluteFill} device={device} isActive={active} outputs={[frameOutput, photoOutput]} zoom={device.neutralZoom} resizeMode="cover" onStarted={cameraReady} onPreviewStarted={cameraReady} onError={(cameraError: any) => { setReady(false); setStatus(cameraError.message); }} />
    ...
    <SafeAreaView style={s.cameraSafe}><View style={s.cameraTop}><Pressable onPress={onClose} style={s.glassButton}><Feather name="x" size={23} color={C.white} /></Pressable><HeaderIdentity title="Scan your card"/><View style={s.glassButton}><Feather name="zap" size={20} color={C.white} /></View></View>
...
function Analyzing() { return <View style={s.page}><LinearGradient colors={[C.ink,'#251044','#11102D']} style={StyleSheet.absoluteFill}/><SafeAreaView style={s.safe}><View style={s.standaloneHeader}><Brand/></View><View style={s.centerContent}><View style={s.analyzeIcon}><MaterialCommunityIcons name="line-scan" size={56} color={C.cyan} /></View><ActivityIndicator color={C.cyan} size="large" style={{marginTop:28}}/><Text style={s.analyzeTitle}>Retrieving card details…</Text><Text style={s.analyzeSub}>Finding its name, set, and collector number</Text></View></SafeAreaView></View>; }

function ScanConfirmation({draft,error,onChange,onConfirm,onRescan,onCancel}:{draft:ScanDraft;error:string;onChange:(draft:ScanDraft)=>void;onConfirm:(draft:ScanDraft)=>Promise<void>;onRescan:()=>void;onCancel:()=>void}) {
  const [submitting,setSubmitting]=useState(false);
  const confirm=async()=>{if(submitting)return;setSubmitting(true);try{await onConfirm(draft);}finally{setSubmitting(false);}};
  const update=(field:'name'|'set'|'number',value:string)=>onChange({...draft,[field]:value});
  return <View style={s.page}><LinearGradient colors={['#1E0C38',C.ink,'#07050C']} style={StyleSheet.absoluteFill}/><KeyboardAvoidingView style={s.safe} behavior={Platform.OS==='ios'?'padding':'height'}><SafeAreaView style={s.safe}><View style={s.confirmTop}><Pressable onPress={onCancel} style={s.back}><Feather name="x" size={21} color={C.white}/></Pressable><HeaderIdentity title="Confirm your card"/><TierBadge/></View><ScrollView contentContainerStyle={s.confirmScroll} automaticallyAdjustKeyboardInsets={false} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS==='ios'?'interactive':'on-drag'} showsVerticalScrollIndicator={false}><View style={s.confirmPreview}><Image source={{uri:draft.imageUri}} style={s.confirmImage} contentFit="contain" cachePolicy="none"/></View><View style={s.confirmFields}><ConfirmField label="CARD NAME" value={draft.name} onChangeText={value=>update('name',value)} placeholder="e.g. Charizard ex" autoCapitalize="words"/><ConfirmField label="SET" value={draft.set} onChangeText={value=>update('set',value)} placeholder="e.g. SV1" autoCapitalize="characters"/><ConfirmField label="NUMBER" value={draft.number} onChangeText={value=>update('number',value)} placeholder="e.g. 171/198" autoCapitalize="characters" returnKeyType="done" onSubmitEditing={confirm}/></View>{!!error&&<View style={s.confirmError}><Feather name="alert-circle" size={15} color="#FF98A2"/><Text style={s.confirmErrorText}>{error}</Text></View>}<Pressable onPress={confirm} disabled={submitting} style={({pressed})=>[s.confirmButton,(pressed||submitting)&&{opacity:.72}]}>{submitting?<ActivityIndicator color={C.ink}/>:<><Text style={s.confirmButtonText}>FIND CARD</Text><Feather name="arrow-right" size={18} color={C.ink}/></>}</Pressable><View style={s.confirmSecondary}><Pressable onPress={onRescan} disabled={submitting} style={s.rescanButton}><Feather name="camera" size={16} color={C.cyan}/><Text style={s.rescanText}>Rescan</Text></Pressable><Pressable onPress={onCancel} disabled={submitting} style={s.cancelButton}><Text style={s.cancelText}>Cancel</Text></Pressable></View></ScrollView></SafeAreaView></KeyboardAvoidingView></View>;
}

function ConfirmField({label,...props}:{label:string}&ComponentProps<typeof TextInput>) { return <View style={s.confirmField}><Text style={s.confirmLabel}>{label}</Text><TextInput {...props} style={s.confirmInput} placeholderTextColor="#71839C" selectionColor={C.cyan} autoCorrect={false}/></View>; }
...
function Matches({ query, cards, onBack, onCollection, onSelect, onSearch, onScan }: {query:string;cards:Card[];onBack:()=>void;onCollection:()=>void;onSelect:(c:Card)=>void;onSearch:(q:string)=>void;onScan:()=>void}) {
  return <View style={s.page}><SafeAreaView style={s.safe}><View style={s.resultTop}><Pressable onPress={onBack} style={s.back}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><HeaderIdentity title="Choose your match"/><TierBadge/></View>
...
const CONFIRM_PREVIEW_HEIGHT=Math.min(Dimensions.get('window').height*.36,310);
...
const s=StyleSheet.create({
  ...
  confirmTop:{paddingHorizontal:18,paddingVertical:10,flexDirection:'row',alignItems:'center',gap:12},confirmTitle:{fontFamily:'Inter_800ExtraBold',fontSize:20,color:C.white,marginTop:2},confirmScroll:{paddingHorizontal:20,paddingBottom:30},confirmPreview:{height:CONFIRM_PREVIEW_HEIGHT,borderRadius:22,backgroundColor:'rgba(10,18,31,.72)',borderWidth:1,borderColor:'rgba(68,215,255,.18)',padding:9,alignItems:'center',justifyContent:'center',overflow:'hidden',...shadow},confirmImage:{width:'100%',height:'100%',borderRadius:15},confirmFields:{gap:10,marginTop:14},confirmField:{height:58,borderRadius:15,borderWidth:1,borderColor:C.line,backgroundColor:C.panel,paddingHorizontal:14,justifyContent:'center'},confirmLabel:{fontFamily:'Inter_700Bold',fontSize:7,color:C.muted,letterSpacing:1.1,marginBottom:2},confirmInput:{height:34,paddingVertical:0,fontFamily:'Inter_700Bold',fontSize:14,color:C.white},confirmError:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:'rgba(244,63,80,.1)',borderWidth:1,borderColor:'rgba(244,63,80,.25)',padding:10,borderRadius:12,marginTop:10},confirmErrorText:{flex:1,fontFamily:'Inter_500Medium',fontSize:10,color:'#FFB0B8'},confirmButton:{height:58,borderRadius:16,backgroundColor:C.yellow,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:9,marginTop:14,...shadow},confirmButtonText:{fontFamily:'Inter_800ExtraBold',fontSize:12,color:C.ink,letterSpacing:.8},confirmSecondary:{height:48,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:28},rescanButton:{flexDirection:'row',alignItems:'center',gap:7,padding:10},rescanText:{fontFamily:'Inter_700Bold',fontSize:11,color:C.cyan},cancelButton:{padding:10},cancelText:{fontFamily:'Inter_600SemiBold',fontSize:11,color:C.muted},
