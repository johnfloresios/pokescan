...
import { ActivityIndicator, Alert, Animated, Dimensions, FlatList, ImageBackground, KeyboardAvoidingView, PanResponder, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View, type ScrollViewProps } from 'react-native';
...
const KeyboardAwareScrollView=forwardRef<ScrollView,ScrollViewProps>(function KeyboardAwareScrollView({children,...props},ref){
  return <KeyboardAvoidingView style={s.safe} behavior={Platform.OS==='ios'?'padding':'height'} keyboardVerticalOffset={Platform.OS==='ios'?8:0}><ScrollView ref={ref} automaticallyAdjustKeyboardInsets contentInsetAdjustmentBehavior="automatic" keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS==='ios'?'interactive':'on-drag'} {...props}>{children}</ScrollView></KeyboardAvoidingView>;
});
const revealBottomInput=(ref:{current:ScrollView|null})=>setTimeout(()=>ref.current?.scrollToEnd({animated:true}),180);
...
function SavedCardControls({row,onUpdate,onDelete,onInputFocus}:{row:ScannedCardRow;onUpdate:(updates:Pick<ScannedCardRow,'quantity'|'condition'|'variant'|'notes'>)=>Promise<void>;onDelete:()=>Promise<void>;onInputFocus:()=>void}) {
