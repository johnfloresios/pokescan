...
function AuthPlaceholder({ configured }: { configured: boolean }) {
  ...
  return <View style={s.page}><LinearGradient colors={['#160D2B',C.ink,'#07050C']} style={StyleSheet.absoluteFill}/><KeyboardAvoidingView style={s.safe} behavior={Platform.OS==='ios'?'padding':'height'}><SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.authScroll} automaticallyAdjustKeyboardInsets contentInsetAdjustmentBehavior="automatic" keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive"><Brand/><View style={s.authMark}><MaterialCommunityIcons name="pokeball" size={42} color={C.yellow}/></View><Text style={s.authTitle}>{mode==='signin'?'Welcome back':'Create your account'}</Text><Text style={s.authCopy}>{mode==='signin'?'Sign in to view your collection and scan history.':'Start tracking the cards you scan and their value.'}</Text><View style={s.authForm}>
...
function CollectionScreen({userId,onBack,onScan,onSelect}:{userId:string;onBack:()=>void;onScan:()=>void;onSelect:(row:ScannedCardRow)=>void}) {
  ...
  return <View style={s.page}><SafeAreaView style={s.safe}><View style={s.collectionTop}><Pressable onPress={onBack} style={s.back}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><View style={{flex:1}}><Text style={s.smallHead}>POKÉSCAN WALLET</Text><Text style={s.resultTitle}>Collection</Text></View><TierBadge/></View>
...
function Analyzing() { return <View style={[s.page, s.center]}><LinearGradient colors={[C.ink,'#251044','#11102D']} style={StyleSheet.absoluteFill}/><View style={s.analyzeIcon}><MaterialCommunityIcons name="line-scan" size={56} color={C.cyan} /></View><ActivityIndicator color={C.cyan} size="large" style={{marginTop:28}}/><Text style={s.analyzeTitle}>Retrieving card details…</Text><Text style={s.analyzeSub}>Finding its name, set, and collector number</Text></View>; }
...
function Matches({ query, cards, onBack, onCollection, onSelect, onSearch, onScan }: {query:string;cards:Card[];onBack:()=>void;onCollection:()=>void;onSelect:(c:Card)=>void;onSearch:(q:string)=>void;onScan:()=>void}) {
  return <View style={s.page}><SafeAreaView style={s.safe}><View style={s.resultTop}><Pressable onPress={onBack} style={s.back}><Feather name="arrow-left" size={22} color={C.white}/></Pressable><View style={{flex:1}}><Text style={s.smallHead}>SCAN COMPLETE</Text><Text style={s.resultTitle}>Choose your match</Text></View><TierBadge/></View>
