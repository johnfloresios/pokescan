...
// confirmation UI both consume this single catalog.
...
export function matchPrintedSetCode(lines:readonly string[]):SetCodeMatch|undefined{
  ...
  let best:SetCodeMatch|undefined;
  for(const token of tokens){
    for(const set of POKEMON_SETS){
      ...
      if(confidence&&(!best||confidence>best.confidence))best={set,confidence,raw:token.raw};
    }
  }
  return best;
}
